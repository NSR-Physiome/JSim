...
// -----------------------------------------------------------------------------
	...
	delx = 0.001 cm;			// Delta x for approx Q
...
//        right hand boundaries using a central difference
	Q_lh = D * ( (3*C_lh*Lambda) - 4*C(x.min+delx,t) +1*C(x.min+2*delx,t))/(2*delx);
	Q_rh = D * (-C(x.max-2*delx,t) +4*C(x.max-delx,t) - 3*(C_rh*Lambda) )/(2*delx);
