...
C
     ...
     &          IP1(NM1),IPHES(N)
      LOGICAL CALHES
      INTEGER iLINAL(7),MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG
      CALL extLINAL(iLINAL,MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG)
...
C
     ...
     &          F2,F3,CONT,IP2,IPHES,IER,IJOB,iLINAL)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION FJAC(LDJAC,N),FMAS(LDMAS,NM1),
...
C
     ...
     &          F1,F2,F3,CONT,IP1,IP2,IPHES,IER,IJOB,iLINAL)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION FJAC(LDJAC,N),FMAS(LDMAS,NM1),E1(LDE1,NM1),
...
C ***********************************************************
C
      SUBROUTINE ESTRAD(ithrndx,
     &          N,FJAC,LDJAC,MLJAC,MUJAC,FMAS,LDMAS,MLMAS,MUMAS,
     &          H,DD1,DD2,DD3,FCN,NFCN,Y0,Y,IJOB,X,M1,M2,NM1,
     &          E1,LDE1,Z1,Z2,Z3,CONT,F1,F2,IP1,IPHES,SCAL,ERR,
     &          FIRST,REJECT,FAC1,RPAR,IPAR,iLINAL)
...
C
      ...
      IF (FIRST.OR.REJECT) THEN
          DO I=1,N
             CONT(I)=Y(I)+CONT(I)
          END DO
          CALL FCN(ithrndx,N,X,CONT,F1,RPAR,IPAR)
          NFCN=NFCN+1
          DO I=1,N
             CONT(I)=F1(I)+F2(I)
          END DO
...
C
      ...
      IF (FIRST.OR.REJECT) THEN
          DO I=1,N
             CONT(I)=Y(I)+CONT(I)
          END DO
          CALL FCN(ithrndx,N,X,CONT,FF,RPAR,IPAR)
          NFCN=NFCN+1
          DO I=1,N
             CONT(I)=FF(I)+FF(I+N)
          END DO
...
C
     ...
     &          M1,M2,NM1,FAC1,E,LDE,IP,IPHES,DEL,IJOB,iLINAL)
      IMPLICIT REAL*8 (A-H,O-Z)
...
C    :::::::::: last card of elmhes ::::::::::
      ...
      SUBROUTINE saveLINAL(iLINAL,MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG)
      INTEGER iLINAL(7),MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG
      iLINAL(1)=MLE
      iLINAL(2)=MUE
      iLINAL(3)=MBJAC
      ...
      RETURN
      END
      SUBROUTINE extLINAL(iLINAL,MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG)
      INTEGER iLINAL(7),MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG
      MLE=iLINAL(1)
      MUE=iLINAL(2)
      MBJAC=iLINAL(3)
