...
public class RTContext extends FgenContext implements NamedVal.Query {
	...
	protected void setDomain(RTRealDomain x) throws Xcept {
	    ...
	    if (getQueryTimeNth(x) > 1)
	    	store().createStoreNth();
	    unSet(x); // must be set before access
	}

	// every Nth point at run-time
	protected int getRunTimeNth(RTRealDomain x) throws Xcept {
	    int nth = getNth(x);
	    if (nth == 1) return nth;
	    if (! runTimeNthSupported(x)) return 1;
	    return nth;
	}
	
	// every Nth point at query-time
	protected int getQueryTimeNth(RTRealDomain x) throws Xcept {
	    int nth = getNth(x);
	    if (nth == 1) return nth;
	    if (runTimeNthSupported(x)) return 1;
	    return nth;
	...
	// is runTime Nth _REALLY_ supported?
	//    this provides local override in case planner calc incorrect
	protected boolean runTimeNthSupported(RTRealDomain x) throws Xcept {

	    // Code below enable run-time reduction in some cases
            //   as of 2Jun14, not all run-time reduction errors could be predicted
    	    //   so for now, all run-time reduction is disabled
	    if (! model.runTimeNthSupported(x)) 
	    	return false;
	    if (store.hasDynamicInput(x))
	    	return false;
	    return true;
