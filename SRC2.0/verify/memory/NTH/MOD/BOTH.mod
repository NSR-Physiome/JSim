...
//        right hand boundaries using a central difference
	Q_lh = D * ( (3*C_lh*Lambda) - 4*C(t,x.min+x.delta) +1*C(t,x.min+2*x.delta))/(2*x.delta);
	Q_rh = D * (-C(t,x.max-2*x.delta) +4*C(t,x.max-x.delta) - 3*(C_rh*Lambda) )/(2*x.delta);
