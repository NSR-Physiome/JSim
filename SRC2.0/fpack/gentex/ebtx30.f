      REAL FUNCTION et30i(cin,z,q0,t0,deltex, mxseg, rwkv, iwkv, lwkv)
c
c Re-entrant 3-region 2-barrier blood-tissue exchange operator
c
c File ebtex30.f (Version 1.6).  Last modified at 11:00:07 on 11/16/95.
c
c.......................................................................
c
...
c SYNOPSIS:
c
c     REAL FUNCTION ebt30i(cin, z, q0, deltex, mxseg, rwkv, iwkv, lwkv)
c     ENTRY         ebt30 (cin, q, time, mxseg, dwkv, rwkv, iwkv, lwkv)
c
...
c DESCRIPTION
c
c ebtex30 is a re-entrant version of btex30, a 3-region 2-barrier
...
c RETURN VALUE:
c
c ebt30i always returns a value of 0.0. ebt30 return the concentration of 
...
c EXAMPLES:
c
c In the following code fragment ebtex30 is used to model 10
...
c       .
c     DO 1 i = 1, NPATH
c         cout(i) = ebt30i(cin(i), z(1,i), q0(i), deltex, MAXSEG,
...
c       .
c     DO 10 i = 1, NPATH
c         cout(i) = ebt30(cin(i), q(i), time, MAXSEG,
...
c ~libnsr/lib/libmath/ebtex30 - source files
c
c.......................................................................
c
c AUTHOR:
...
c
c WORKING STORAGE ORGANIZATION (NREG=3):
c
...
c
      DATA         sid1
     + /'@(#)ebtex30.f	1.6 created on 11/16/95 at 11:00:07.'/
      DATA         sid2
     + /'@(#) Retrieved on 3/20/97 at 17:47:39.'/
c
c-------------------------------------------------INITIALIZATION SECTION
c
...
c-------------------------------------------------------SOLUTION SECTION
c
      ENTRY et30(cin, q, time, mxseg, dwkv, rwkv, iwkv, lwkv)
c
c Since the EBTEX30 internal time step & the external time step are not
...
c
      ...
      ENDIF
c
c IV. Update the input integral
c
      rwkv(ICIN)  = cin
...
c
          DO 220 j = 1,iwkv(INSEG)
              ...
              C2 = rwkv(IOFFC2+j)
              C3 = rwkv(IOFFC3+j)
              rwkv(IOFFC1+j) = CF11*C1 + CF12*C2 + CF13*C3
              rwkv(IOFFC2+j) = CF21*C1 + CF22*C2 + CF23*C3
              rwkv(IOFFC3+j) = CF31*C1 + CF32*C2 + CF33*C3
  220     CONTINUE
c
...
c
  300 CONTINUE
      ...
      et30       = MAX(0.0,(rwkv(IINTO)+oc-rwkv(IOCZER))/rwkv(ISMDT))
c
