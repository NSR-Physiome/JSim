      SUBROUTINE ggopt(f,nx,x,maxit,grdtl,stptl,iout,fmin,eps,wk,istop)
c
...
c     INTEGER nx, maxit, iout, istop
c     REAL x(nx),grdtl,stptl,fmin,eps
c     REAL wk((1+nx+nx*nx)*(1+nx+nx*(nx+1)/2))
c     EXTERNAL f
c     CALL ggopt(f,nx,x,maxit,grdtl,stptl,iout,fmin,eps,wk,istop)
c
c INPUTS:
c     f:     a real function subprogram supplied by the user.  
c            f must be declared external in the calling program.  
c            f defines the function to be minimized and should 
c            be of the form:   f(x).
...
c            istop = 5, the function value is less or equal to fmin.
c
c WORKING STORAGE:
...
c
      ...
      PARAMETER (MXDIM=10, MXCOL=1+MXDIM+MXDIM*(MXDIM+1)/2)
      PARAMETER (MXROW=1+MXDIM+MXDIM*MXDIM)
c
c     B. Declare global variables
c
      INTEGER nx, maxit, info, iout, istop
      REAL x(*), grdtl, stptl, fmin, eps, wk(*)
c
c     C. Declare local variables
c
      INTEGER n, nloop, i, j 
      REAL x0(MXDIM), h(MXDIM), s(MXDIM), theta(MXCOL)
      REAL wk1(MXROW,MXDIM), wk3(MXROW), wk4(MXCOL)
      REAL  dnorm, stplth, stepln, fplus, fzero, rstep, epsmch
      CHARACTER*64 sid1, sid2
      REAL f, r1mach, snrm2
      EXTERNAL covar, f, mesh, r1mach, saxpy, scopy, snrm2, stepln
c
...
c
      IF(iout.GE.0)WRITE(iout, 1) maxit
    1 FORMAT(' maxit = ',i6)
...
c
      ...
      fplus=f(x)
      fzero=fplus
      CALL scopy(n,x,1,x0,1)
c
c     C. fetch machine epsilon
c        Note: If your machine does not have r1mach routine, or
c              the r1mach routine provided does not give the
...
c
          CALL mesh(f,n,x0,h,eps,fplus,theta,wk1,wk,wk3,s,wk4)
c
c        C. compute the Euclidean norm of the gradient
c
          dnorm = snrm2(n, theta(2), 1)
...
c
          stplth=stepln(f,x0,n,s,theta,fplus,wk1,wk4,epsmch)
c
c        E. update the parameter x=x-stplth*s, and compute relative
c           step changes
c
          CALL saxpy(n, -stplth, s, 1, x0, 1)
          DO 99 j=1,n
              wk3(j)=(x0(j)-x(j))
...
c
  ...
  995 istop=5
      IF(iout.GE.0)
     +   WRITE(iout,'('' Function value is less than fmin'')')
  999 CONTINUE
c
...
c
 ...
 1000     CONTINUE
      ENDIF
      IF(iout.GE.0) WRITE(iout,40)dnorm,wk(1),(x(i),i=1,n)
      RETURN
      END
c
c -----------------------------------------------------------------
c
      SUBROUTINE mesh(f,n,x0,h0,eps,f0,theta,xp,a,y,x1,sc)
c
...
c CALL AND PARAMETER TYPES:
c     INTEGER n
c     REAL x0(n), h0(n), eps, f0, theta(1+n+n*(n+1)/2)
c     REAL xp(1+n+n*n,n), a(1+n+n*n,1+n+(n+1)*n/2), y(1+n+n*n)
c     REAL x1(n), sc(1+n+n*(n+1)/2)
c     EXTERNAL f
c     CALL mesh(f,n,x0,h0,eps,f0,theta,xp,a,y,x1,sc)
c
c INPUTS:
...
c
      INTEGER n
      REAL h0(n), x0(n), theta(*), f0, eps
      REAL xp(1+n+n*n,*), a(1+n+n*n,1+n+(n+1)*n/2), y(*)
      REAL x1(n), sc(*)
c
c     B. Declare local variables
c
      INTEGER n1, n2, i, j, k, icntz, jpvt, info
      REAL qy, rsd, xb, df, factor
      REAL f
c
c     C. Declare externals 
c
      EXTERNAL f, scopy, sqrdc, sqrsl
c
...
c      values of function, gradient and Hessian.
c  
      CALL sqrdc(a,n2,n2,n1,xp(1,1),jpvt,xp(1,2),0)
      CALL sqrsl(a,n2,n2,n1,xp(1,1),y,qy,xp(1,2),theta,rsd,xb,
     +           100, info)
c
...
c ----------------------------------------------------------------
c
      REAL FUNCTION stepln(f,x0,n,s,theta,fplus,hes,x1,epsmch)
c   
...
c CALL AND PARAMETER TYPES:
c     INTEGER n
c     REAL x0(n), s(n), theta(1+n+n*(n+1)/2), fplus, hes(n,n),x1(n)
c     EXTERNAL f
...
c
      INTEGER n
      REAL x0(n), theta(*), s(n), fplus, hes(n,n), x1(n), epsmch
c
c     B. Declare local variables
c
      REAL r, fzero
...
c CALL AND PARAMETER TYPES:
c     INTEGER n
c     REAL theta(1+n+n*(n+1)/2), s(n), a(n,n)
...
c     REAL theta(1+n+n*(n+1)/2), a(lda,n)
