...
public class ColumnDataReader extends TextDataReader {
	...
	private StringList yunames; 
	private ArrayList<double[]> lineData; // build area for line data

	// constructor
	public ColumnDataReader(ColumnDataFormat f, Reader r) 
	throws Xcept {
	    super(f, r);
	}
	...
	public Data.List readData() throws Xcept {
	    ...
	    // process second line tokens (units or data)
	    yunames = new StringList(ny);
	    line = readLine();
 	    toks = getTokens(line);
		...
		xuname = peelQuote(toks.str(0));
		for (int i=0; i<ny; i++) {
		    String tok = toks.str(1+i);
		    yunames.add(peelQuote(tok));
		}
	    }	    
	...
	private Data.List makeData() throws Xcept {
	    ...
	    Unit unit = Util.isBlank(xuname) ? 
		null : new Unit(xuname);
	    ...
	    GridData xgrid = new IrregularGridData(xname, unit, xsamp);

	    // convert to regular grid?
	    RegularGridData rgrid = new RegularGridData(xname, unit,
	    ...
	    for (int i=0; i<ny; i++) {
		String uname = (i < yunames.size()) ?
		    yunames.str(i) : null;
		unit = Util.isBlank(uname) ? 
		    null : new Unit(uname);
		Data ydata = new RealNData("", unit, 
		    new GridData[] { xgrid });
