 ...
 *                                                                 *
 * Modification July 13, 2004 by Gary M. Raymond                   *
 * fprintf statements changed to jsfprintf                         *
 * If output file is NULL, output not written to stdout.           *
 *                                                                 *
 *******************************************************************/
                  ...
                  long int iopt[], realtype ropt[], M_Env machEnv)
{
  ...
  fp = (errfp == NULL) ? errfp  : errfp;

  if (y0==NULL) {
    jsfprintf(fp, MSG_Y0_NULL);
    return(NULL);
  }
  
  if (N <= 0) {
    jsfprintf(fp, MSG_BAD_N, N);
    return(NULL);
  }

  if ((lmm != ADAMS) && (lmm != BDF)) {
    jsfprintf(fp, MSG_BAD_LMM, lmm, ADAMS, BDF);
    return(NULL);
  }

  if ((iter != FUNCTIONAL) && (iter != NEWTON)) {
    jsfprintf(fp, MSG_BAD_ITER, iter, FUNCTIONAL, NEWTON);
    return(NULL);
  }

  if ((itol != SS) && (itol != SV)) {
    jsfprintf(fp, MSG_BAD_ITOL, itol, SS, SV);
    return(NULL);
  }

  if (f == NULL) {
    jsfprintf(fp, MSG_F_NULL);
    return(NULL);
  }

  if (reltol == NULL) {
    jsfprintf(fp, MSG_RELTOL_NULL);
    return(NULL);
  }

  if (*reltol < ZERO) {
    jsfprintf(fp, MSG_BAD_RELTOL, *reltol);
    return(NULL);
  }
   
  if (abstol == NULL) {
    jsfprintf(fp, MSG_ABSTOL_NULL);
    return(NULL);
  }
  ...
  if (neg_abstol) {
    jsfprintf(fp, MSG_BAD_ABSTOL);
    return(NULL);
  }

  if ((optIn != FALSE) && (optIn != TRUE)) {
    jsfprintf(fp, MSG_BAD_OPTIN, optIn, FALSE, TRUE);
    return(NULL);
  }

  if ((optIn) && (iopt == NULL) && (ropt == NULL)) {
    jsfprintf(fp, MSG_BAD_OPT);
    return(NULL);
  } 
             ...
             long int iopt[], realtype ropt[], M_Env machEnv) 
{
  ...
  if (optIn && ioptExists)
    { if (iopt[MAXORD] > 0)  maxord = MIN(maxord, iopt[MAXORD]); }
  if (maxord > qmax) {
    jsfprintf(fp, MSG_REI_MAXORD, qmax, maxord);
    return(CVREI_ILL_INPUT);
  }
          ...
          realtype *t, int itask)
{
  ...
  cv_mem = (CVodeMem) cvode_mem;
  if (cvode_mem == NULL) {
    jsfprintf(errfp , MSG_CVODE_NO_MEM);
    return(CVODE_NO_MEM);
  }
  
  if ((y = yout) == NULL) {
    jsfprintf(errfp, MSG_YOUT_NULL);       
    return(ILL_INPUT);
  }
  
  if (t == NULL) {
    jsfprintf(errfp, MSG_T_NULL);
    return(ILL_INPUT);
  }
  *t = tn;

  if ((itask != NORMAL) && (itask != ONE_STEP)) {
    jsfprintf(errfp, MSG_BAD_ITASK, itask, NORMAL, ONE_STEP);
    return(ILL_INPUT);
  }
  ...
  if (optIn && ropt != NULL) {
    ...
    if (hmin*hmax_inv > ONE) {
      jsfprintf(errfp, MSG_BAD_HMIN_HMAX, hmin, ropt[HMAX]);
      return(ILL_INPUT);
    }
  }
  ...
  if (nst == 0) {
    if (iter == NEWTON) {
      if (linit == NULL) {
        jsfprintf(errfp, MSG_LINIT_NULL);
        return(ILL_INPUT);
      }
      if (lsetup == NULL) {
        jsfprintf(errfp, MSG_LSETUP_NULL);
        return(ILL_INPUT);
      }
      if (lsolve == NULL) {
        jsfprintf(errfp, MSG_LSOLVE_NULL);
        return(ILL_INPUT);
      }
      if (lfree == NULL) {
        jsfprintf(errfp, MSG_LFREE_NULL);
        return(ILL_INPUT);
      }
      ier = linit(cv_mem);
      if (ier != LINIT_OK) {
        jsfprintf(errfp, MSG_LINIT_FAIL);
        return(ILL_INPUT);
      }
    }
    ...
    if ( (h != ZERO) && ((tout-tn)*h < ZERO) ) {
      jsfprintf(errfp, MSG_BAD_H0, h, tout-tn);
      return(ILL_INPUT);
    }
    if (h == ZERO) {
      hOK = CVHin(cv_mem, tout);
      if (!hOK) {
        jsfprintf(errfp, MSG_TOO_CLOSE, tout, tn);
        return(ILL_INPUT);
      }
    }
    rh = ABS(h)*hmax_inv;
  ...
  if ( (itask == NORMAL) && (nst > 0) && ((tn-tout)*h >= ZERO) ) {
    *t = tout;
    ...
    if (ier != OKAY) {  /* ier must be == BAD_T */
      jsfprintf(errfp, MSG_BAD_TOUT, tout);
      return(ILL_INPUT);
    }
    return(SUCCESS);
  }
  ...
  loop {
    ...
    if (nst > 0) {
      ...
      if (!ewtsetOK) {
        jsfprintf(errfp, MSG_EWT_NOW_BAD, tn);
        istate = ILL_INPUT;
        *t = tn;
    ...
    if (nstloc >= mxstep) {
      jsfprintf(errfp, MSG_MAX_STEPS, tn, mxstep, tout);
    ...
    if ((tolsf = uround * N_VWrmsNorm(zn[0], ewt)) > ONE) {
      jsfprintf(errfp, MSG_TOO_MUCH_ACC, tn);
      istate = TOO_MUCH_ACC;
      *t = tn;
      N_VScale(ONE, zn[0], yout);
      tolsf *= TWO;
    ...
    if (tn + h == tn) {
      nhnil++;
      if (nhnil <= mxhnil) jsfprintf(errfp, MSG_HNIL, tn, h);
      if (nhnil == mxhnil) jsfprintf(errfp, MSG_HNIL_DONE, mxhnil);
    }
...
int CVodeDky(void *cvode_mem, realtype t, int k, N_Vector dky)
{
  ...
  if (cvode_mem == NULL) {
    jsfprintf(errfp , MSG_DKY_NO_MEM);
    return(DKY_NO_MEM);
  }
  
  if (dky == NULL) {
    jsfprintf(errfp , MSG_BAD_DKY);
    return(BAD_DKY);
  }

  if ((k < 0) || (k > q)) {
    jsfprintf(errfp, MSG_BAD_K, k);
    return(BAD_K);
  }
  ...
  tn1 = tn + tfuzz;
  if ((t-tp)*(t-tn1) > ZERO) {
    jsfprintf(errfp, MSG_BAD_T, t, tn-hu, tn);
    return(BAD_T);
  }
...
static int CVHandleFailure(CVodeMem cv_mem, int kflag)
{
  ...
  switch (kflag) {
    case REP_ERR_FAIL:  jsfprintf(errfp, MSG_ERR_FAILS, tn, h);
                        return(ERR_FAILURE);
    case REP_CONV_FAIL: jsfprintf(errfp, MSG_CONV_FAILS, tn, h);
                        return(CONV_FAILURE);
    case SETUP_FAILED:  jsfprintf(errfp, MSG_SETUP_FAILED, tn);
                        return(SETUP_FAILURE);
    case SOLVE_FAILED:  jsfprintf(errfp, MSG_SOLVE_FAILED, tn);
