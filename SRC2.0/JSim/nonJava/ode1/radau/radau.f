      SUBROUTINE RADAU(ithrndx,N,FCN,X,Y,XEND,H,
     &                  RTOL,ATOL,ITOL,
...
C     INPUT PARAMETERS  
C     ----------------  
c     ithrndx     thread index
C     N           DIMENSION OF THE SYSTEM 
C
C     FCN         NAME (EXTERNAL) OF SUBROUTINE COMPUTING THE
C                 VALUE OF F(X,Y):
C                    SUBROUTINE FCN(ithrndx,N,X,Y,F,RPAR,IPAR)
...
C                    SUBROUTINE MAS(ithrndx,N,AM,LMAS,RPAR,IPAR)
...
cQ          WRITE(6,*)' WRONG INPUT IWORK(13)=',IWORK(13)
            ARRET=.TRUE.
         END IF
      END IF
C -------- NMAX , THE MAXIMAL NUMBER OF STEPS -----
      ...
      ELSE
         NMAX=IWORK(2)
         IF (NMAX.LE.0) THEN
cQ          WRITE(6,*)' WRONG INPUT IWORK(2)=',IWORK(2)
            ARRET=.TRUE.
         END IF
      END IF
...
cQ          WRITE(6,*)' CURIOUS INPUT IWORK(3)=',IWORK(3)
            ARRET=.TRUE.
         END IF
      END IF
...
cQ     WRITE(6,*)' CURIOUS INPUT FOR IWORK(5,6,7)=',NIND1,NIND2,NIND3
       ARRET=.TRUE.
      END IF
...
cQ     WRITE(6,*)' CURIOUS INPUT FOR IWORK(9,10)=',M1,M2
       ARRET=.TRUE.
      END IF
...
cQ          WRITE(6,*)' COEFFICIENTS HAVE 20 DIGITS, UROUND=',WORK(1)
            ARRET=.TRUE.
         END IF
      END IF
...
cQ            WRITE (6,*) ' TOLERANCES ARE TOO SMALL'
              ARRET=.TRUE.
          END IF
      ELSE
          DO I=1,N
          IF (ATOL(I).LE.0.D0.OR.RTOL(I).LE.10.D0*UROUND) THEN
cQ            WRITE (6,*) ' TOLERANCES(',I,') ARE TOO SMALL'
              ARRET=.TRUE.
          END IF
          END DO
      END IF
...
cQ          WRITE(6,*)' CURIOUS INPUT FOR WORK(2)=',WORK(2)
            ARRET=.TRUE.
         END IF
      END IF
...
cQ          WRITE(6,*)' CURIOUS INPUT FOR WORK(3)=',WORK(3)
            ARRET=.TRUE.
         END IF
      END IF
...
cQ       WRITE(6,*)' CURIOUS INPUT FOR WORK(5,6)=',QUOT1,QUOT2
         ARRET=.TRUE.
      END IF
C -------- MAXIMAL STEP SIZE
...
cQ          WRITE(6,*)' CURIOUS INPUT WORK(8,9)=',WORK(8),WORK(9)
            ARRET=.TRUE.
         END IF
...
cQ           WRITE (6,*) 'BANDWITH OF "MAS" NOT SMALLER THAN BANDWITH OF
cQ   & "JAC"'
            ARRET=.TRUE.
          END IF
      ELSE
...
cQ       WRITE(6,*)' HESSENBERG OPTION ONLY FOR EXPLICIT EQUATIONS WITH 
cQ   &FULL JACOBIAN'
         ARRET=.TRUE.
      END IF
...
cQ       WRITE(6,*)' INSUFFICIENT STORAGE FOR WORK, MIN. LWORK=',ISTORE
         ARRET=.TRUE.
      END IF
...
cQ       WRITE(6,*)' INSUFF. STORAGE FOR IWORK, MIN. LIWORK=',ISTORE
         ARRET=.TRUE.
      END IF
...
C -------- CALL TO CORE INTEGRATOR ------------
      CALL RADCOV(ithrndx,N,FCN,X,Y,XEND,HMAX,H,RTOL,ATOL,ITOL,NSUS,
     ...
     &   NREJCT,NDEC,NSOL,RPAR,IPAR,iLINAL)
      IWORK(13)=NSUS
      IWORK(14)=NFCN
      IWORK(15)=NJAC
      IWORK(16)=NSTEP
...
C
     ...
     &   NFCN,NJAC,NSTEP,NACCPT,NREJCT,NDEC,NSOL,RPAR,IPAR,iLINAL)
C ----------------------------------------------------------
...
C ------- COMPUTE MASS MATRIX FOR IMPLICIT CASE ----------
      IF (IMPLCT) CALL MAS(ithrndx,NM1,FMAS,LDMAS,RPAR,IPAR)
...
C ---------- CONSTANTS ---------
 ...
 3142 CONTINUE
     ...
     &                NN,NSCON,XSOL,HSOL,C)
      ...
      MDIFF=MLE+MUE-MUMAS
      MBDIAG=MUMAS+1
      CALL saveLINAL(iLINAL,MLE,MUE,MBJAC,MBB,MDIAG,MDIFF,MBDIAG)
      ...
      END IF
      HHFAC=H
      CALL FCN(ithrndx,N,X,Y,Y0,RPAR,IPAR)
      NFCN=NFCN+1
...
C --- JACOBIAN IS BANDED
 ...
 12               FF(J)=Y(J)
                  ...
                  CALL FCN(ithrndx,N,X,Y,CONT,RPAR,IPAR)
                  J=K+(MM-1)*M2
                  J1=K
...
C --- COMPUTE JACOBIAN MATRIX ANALYTICALLY
         CALL JAC(ithrndx,N,X,Y,FJAC,LDJAC,RPAR,IPAR)
      END IF
      CALJAC=.TRUE.
      CALHES=.TRUE.
  20  CONTINUE
...
C --- COMPUTE THE MATRICES E1 AND E2 AND THEIR DECOMPOSITIONS
     ...
     &         LDE1,IP2((K-1)*NM1+1),IER,IJOB,iLINAL)
         IF (IER.NE.0) GOTO 78
      END DO
      NDEC=NDEC+1
  30  CONTINUE
...
C ---     COMPUTE THE RIGHT-HAND SIDE
      DO I=1,N
         CONT(I)=Y(I)+ZZ(I)
      END DO
      CALL FCN(ithrndx,N,X+C31*H,CONT,ZZ,RPAR,IPAR)
      DO I=1,N
         CONT(I)=Y(I)+ZZ(I+N)
      END DO
      CALL FCN(ithrndx,N,X+C32*H,CONT,ZZ(1+N),RPAR,IPAR)
      DO I=1,N
         CONT(I)=Y(I)+ZZ(I+N2)
      END DO
      CALL FCN(ithrndx,N,XPH,CONT,ZZ(1+N2),RPAR,IPAR)
      NFCN=NFCN+3
C ---     SOLVE THE LINEAR SYSTEMS
     ...
     &          CONT,IP1,IP2,IPHES,IER,IJOB,iLINAL)
      NSOL=NSOL+1
      NEWT=NEWT+1
      DYNO=0.D0
      DO I=1,N
...
C --- ERROR ESTIMATION  
     ...
     &          IPHES,SCAL,ERR,FIRST,REJECT,FAC1,RPAR,IPAR,iLINAL)
...
C ---     COMPUTE THE RIGHT-HAND SIDE
      DO K=0,NS-1
         ...
         END DO
         CALL FCN(ithrndx,N,X+C(K+1)*H,CONT,ZZ(IADD+1),RPAR,IPAR)
      END DO
      NFCN=NFCN+NS
C ---     SOLVE THE LINEAR SYSTEMS
      DO I=1,N
     ...
     &     FF(1+2*K*N),CONT,IP2((K-1)*NM1+1),IPHES,IER,IJOB,iLINAL)
      END DO
      NSOL=NSOL+1
      NEWT=NEWT+1
      DYNO=0.D0
...
C ---     COMPUTE THE RIGHT-HAND SIDE
      DO I=1,N
         CONT(I)=Y(I)+ZZ(I)
      END DO
      CALL FCN(ithrndx,N,XPH,CONT,ZZ,RPAR,IPAR)
      NFCN=NFCN+1
C ---     SOLVE THE LINEAR SYSTEMS
      CALL SLVRAR(N,FJAC,LDJAC,MLJAC,MUJAC,FMAS,LDMAS,MLMAS,MUMAS,
     &          M1,M2,NM1,FAC1,E1,LDE1,ZZ,FF,IP1,IPHES,IER,IJOB,
...
C --- FAIL EXIT
 176  CONTINUE
cQ    WRITE(6,979)X   
cQ    WRITE(6,*) ' MATRIX IS REPEATEDLY SINGULAR, IER=',IER
      IDID=-4
      RETURN
 177  CONTINUE
cQ    WRITE(6,979)X   
cQ    WRITE(6,*) ' STEP SIZE T0O SMALL, H=',H
      IDID=-3
      RETURN
 178  CONTINUE
cQ    WRITE(6,979)X   
cQ    WRITE(6,*) ' MORE THAN NMAX =',NMAX,'STEPS ARE NEEDED' 
      IDID=-2
      RETURN
