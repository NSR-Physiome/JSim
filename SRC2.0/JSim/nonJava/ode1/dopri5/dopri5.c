/* dopri5.c : see comments in dopri5.h */	
...
double hinit (int threadInx, unsigned n, FcnEqDiff fcn, double x, double* y,
...
/* core integrator */
int dopcor (int threadInx,
		   unsigned n, FcnEqDiff fcn, double x, double* y, double xend,
		   ...
		   double *wk14n, unsigned nrds)
{
  ...
  // EB changes Aug 06: decode wk14n
  rcont1 = wk14n + 0*n; 
  rcont2 = wk14n + 1*n; 
  rcont3 = wk14n + 2*n; 
  rcont4 = wk14n + 3*n; 
  rcont5 = wk14n + 4*n; 
  indir = (unsigned *) (wk14n + 5*n);
  yy1 = wk14n + 6*n;
  k1 = wk14n + 7*n;
  k2 = wk14n + 8*n;
  k3 = wk14n + 9*n;
  ...
  hlamb = 0.0;
  iasti = 0;
  fcn (threadInx, n, x, y, k1);
  if (k1[0] != k1[0]) {
	/* printf("Dopri5: detect k1=NaN\n"); */
	return -5;
  }
  hmax = fabs (hmax);
  iord = 5;
  if (h == 0.0)
    h = hinit (threadInx, n, fcn, x, y, posneg, k1, k2, k3, iord, hmax, atoler, rtoler, itoler);
  nfcn += 2;
  reject = 0;
  xold = x;
  if (iout)
  {
    ...
    {
    /*
	printf("Exit of dopri5 at x = %.16e\r\n", x);
	*/
      return 2;
    }
  }
  ...
  while (1)
  {
    ...
    {
    /*
	printf("Exit of dopri5 at x = %.16e, more than nmax = %li are needed\r\n", x, nmax);
	*/
      xout = x;
      hout = h;
      return -2;
	...
	*/
    ...
    fcn (threadInx, n, x+c2*h, yy1, k2);
    if (k2[0] != k2[0]) {
	/* printf("Dopri5: detect k2=NaN\n"); */
	return -5;
    }
    for (i = 0; i < n; i++)
      yy1[i] = y[i] + h * (a31*k1[i] + a32*k2[i]);
    fcn (threadInx, n, x+c3*h, yy1, k3);
    if (k3[0] != k3[0]) {
	/* printf("Dopri5: detect k3=NaN\n"); */
	return -5;
    }
    for (i = 0; i < n; i++)
      yy1[i] = y[i] + h * (a41*k1[i] + a42*k2[i] + a43*k3[i]);
    fcn (threadInx, n, x+c4*h, yy1, k4);
    if (k4[0] != k4[0]) {
	/* printf("Dopri5: detect k4=NaN\n"); */
	return -5;
    }
    for (i = 0; i <n; i++)
      yy1[i] = y[i] + h * (a51*k1[i] + a52*k2[i] + a53*k3[i] + a54*k4[i]);
    fcn (threadInx, n, x+c5*h, yy1, k5);
    if (k5[0] != k5[0]) {
	/* printf("Dopri5: detect k5=NaN\n"); */
	return -5;
    }
    for (i = 0; i < n; i++)
      ysti[i] = y[i] + h * (a61*k1[i] + a62*k2[i] + a63*k3[i] + a64*k4[i] + a65*k5[i]);
    xph = x + h;
    fcn (threadInx, n, xph, ysti, k6);
    if (k6[0] != k6[0]) {
	/* printf("Dopri5: detect k2=NaN\n"); */
	return -5;
    }
    ...
    fcn (threadInx, n, xph, yy1, k2);
    if (k2[0] != k2[0]) {
	/* printf("Dopri5: detect k2=NaN (2)\n"); */
	return -5;
    }
    if (iout == 2)
      if (nrds == n)
	...
	if (hlamb > 3.25)
	{
	  ...
	  if (iasti == 15) {
	    return -4;
	    /*
	      printf("The problem seems to become stiff at x = %.16e\r\n", x);
	    else
	    {
	      xout = x;
	      hout = h;
  ...
  long nmax, int meth, long nstiff, unsigned nrdens, unsigned* icont, unsigned licont)
{
  ...
  /* initialisations */
  arret = 0;
  wk14n = (double *) calloc(14*n, sizeof(double));
  indir = (unsigned *) (wk14n + 5*n);
  ...
  else if (nrdens)
  {
	...
	*/
    ...
    {
    /*
	printf("Insufficient storage for icont, min. licont = %u\r\n", nrdens);
	*/
      arret = 1;
    }
    else
    ...
    /*
      if ((iout < 2))
	printf("Warning : put iout = 2 for dense output\r\n");
	*/
      nrds = nrdens;
      for (i = 0; i < n; i++)
	indir[i] = UINT_MAX;
  ...
  {
    idid = dopcor (threadInx, n, fcn, x, y, xend, hmax, h, rtoler, atoler, itoler, 
		   solout, iout, nmax, uround, meth, nstiff, safe, beta, 
		   fac1, fac2, icont, wk14n, nrds);
    free(wk14n);
    return idid;
  }
