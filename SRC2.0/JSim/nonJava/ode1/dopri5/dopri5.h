...
Source:  http://www.unige.ch/math/folks/hairer/software.html

Revisions:

14 Aug 2003 - Erik Butterworth:
   remove FILE references so can compile under cygwin gcc
     without linking to cygwin.dll
     replaced "fprintf(FILE," with "printf("

18 Mar 2004 - Erik Butterworth:
   check ydot[0] for NaN after each fcn() call
   if so, return -5 (see modified dopri5.h) 
...
dopri5 returns the following values
	...
	-5 : aborted because fcn() returned NaNs 
		(-5 added by Erik Butterworth 16 Mar 2004)
...
typedef void (*FcnEqDiff)(int threadInx, unsigned n, double x, double *y, double *f);
typedef void (*SolTrait)(long nr, double xold, double x, double* y, unsigned n, int* irtrn);


extern int dopri5
 (int threadInx,   /* index of concurrent thread EB Aug 06 */
