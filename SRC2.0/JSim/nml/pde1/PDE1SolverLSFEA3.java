 ...
 *  by GMR
 *
 *  Lagrangian sliding fluid element PDE solver
 *  This is a major revision, converting the algorithm to
 *  use the same number of gridpoints as the other PDE solvers.
 *  The number of grid points can be either odd or even.
 */
...
public class PDE1SolverLSFEA3 extends PDE1Slave
implements ODE1Callbacks {
    public final static String solverName = "LSFEA3";
    ...
    private double[] aleft;
    private double[] aright;
    
    // constructor
    public PDE1SolverLSFEA3(PDE1Solver s) throws Xcept {
	super(s, solverName);
	...
	//B.C.
	int npdo = u0.length;
	double[] aleft = new double[npdo];
	double[] aright = new double[npdo];
	...
	oneTStep(ctxt, t0, tfinal, xgrid, uf, utf, uxf, uxxf, aleft, aright);
    }
    ...
    double[][] ut, double[][] ux, double[][] uxx, double[] aleft, double[] aright)
    throws Xcept {
	...
	gridToSeg(neqn, useg, u, aleft, aright);
	...
	if (nstep > 0) {
		...
		for (int i = 0; i < neqn; i++) {
			...
			xitg[i] = xtmp;
			if (vel[i] > 0)
			{ useg[i][0] = uavg; aleft[i]=uavg;  }
			else if (vel[i] < 0)
			{   useg[i][nseg+1] = uavg; aright[i]=uavg;    }
			
			oamt = slide(useg[i], velRatio[i], tStepSize, true);
			aleft[i]=useg[i][0];
			aright[i]=useg[i][nseg-1];
			oitg[i] += oamt;
		    }
		}
    ...
    public void gridToSeg(int neqn, double[][] useg, double[][] u, double[] aleft, double[] aright) {
